...
const removeParticipantFromGroup = async (req, res) => {
  ...
  try {
    ...
    // If Admin leaving the group then make someone else admin
    if (isAdmin && isSameUser) {
      // Check if the participants are more than 1
      if (group[0].participants.length > 1) {
        // Make someone else Admin
        const newAdminId =
          group[0].participants[0]._id === participantId.toString()
            ? group[0].participants[1]._id
            : group[0].participants[0]._id;
        await Chat.findByIdAndUpdate(chatId, {
          $pull: {
            participants: participantId,
          },
